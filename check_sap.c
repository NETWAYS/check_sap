 ...
 *
 * check_sap -d <destination e.g. SAPID> -c <client e.g. 000> -u <user> -p
 * <pass> [-l <lang>] [-3|-2 (version)] [-t (trace on)] \ -h <hostname> -s
 * <sysnr e.g. 00> [-g <gateway (if not hostname)>] [-x TCP/IP service default
 * sapgwNN (where NN is sysnr)]
 *  [-m <modename>]
 *
 */
 ...
 *
 * The alternative is to exclusively use the ...U/U16 functions available in the
 library
...
#define STRCPY(x, y)                                                           \
	memcpy(x, y, (sizeofR(y) < sizeofR(x)) ? sizeofR(y) : sizeofR(x))
#define STRCPYSTR(x, y)                                                        \
	memcpy(x, y, (strlen(y) < sizeofR(x)) ? strlen(y) : sizeofR(x))
#define STRCPYUSTR(x, y)                                                       \
	memcpy(x, y, (mystrlen(y) < sizeofR(x)) ? mystrlen(y) : sizeofR(x))
...
/* #define SNPRINTF(buf,fmt,str) if (snprintf(buf, sizeof(buf), fmt, str) >=
 * sizeof(buf)) { fprintf(stderr, "Buffer too small at line %d\n", __line__); }
 */
...
int bind_struct_to_table(RFC_TABLE *, SAP_UC *, RFC_TABLE_HANDLE, size_t,
						 RFC_TYPEHANDLE);
...
RFC_RC call_function(RFC_HANDLE, RFC_FUNCTION_HANDLE, RFC_PARAMETER *,
					 RFC_PARAMETER *, RFC_TABLE *, RFC_ERROR_INFO *);
unsigned count_table_rows(RFC_TABLE_HANDLE);
int fill_table(RFC_FUNCTION_HANDLE, SAP_UC *, RFC_TABLE_HANDLE *,
			   RFC_ERROR_INFO *);
...
int main(int argc, char **argv) {
	...
	while ((opt = getopt_long(argc, argv, "c:d:fg:h:jl:m:p:s:tu:x:23C:",
							  longopts, NULL)) != -1) {
		switch (opt) {
		case 'c':
	...
	case LISTTREE:
		xmi_logon(handle);
		tree_items =
			xmi_getmontree(handle, user, monitor_set, monitor_name, &tree);
	...
	case CHECK:
...
#ifdef CACHE
		if (!(tree_items =
				  tree_from_cache(monitor_set, monitor_name, cache, &tree))) {
#endif /* CACHE */
			tree_items =
				xmi_getmontree(handle, user, monitor_set, monitor_name, &tree);
#ifdef CACHE
		...
		if (count > 0) {
			...
			for (i = 0; i < count; i++) {
...
#ifdef DEBUG
				printf("  DBG i=%d,val=%d,last=%u  ", i, data.Value,
					   perfval.Lastalstat);
#endif /* DEBUG */
				...
				if (data.Value != 0) {
					...
					if (output_lines++ == 1) {
						/* we have more than 1 result -> switch to multi-line
						 * output mode */
					...
					snprintf(tmpbuf, sizeof(tmpbuf), "%s%c", sap_options.dest,
							 PATH_SEPARATOR);
					STRADD(output, tmpbuf);
					...
					if (!memcmp(mteptr2->tid.Mtclass, MT_CLASS_PERFORMANCE,
								sizeof(mteptr2->tid.Mtclass))) {
						rc = xmi_get_perfdata(handle, user, current, &perfval);
						rc +=
							xmi_get_perfprop(handle, user, current, &perfprop);
						if (rc != 0) {
							/* error */
							printfU(cU("Failed to retrieve perfdata for %s in "
									   "%s/%s\n"),
						...
						snprintf(tmpbuf, sizeof(tmpbuf),
								 ": %d(warn %d ~ %d, crit %d ~ %d)",
								 perfval.Alrelevval, perfprop.Treshg2y,
								 perfprop.Treshy2g, perfprop.Treshy2r,
								 perfprop.Treshr2y);
#else
						snprintf(tmpbuf, sizeof(tmpbuf), ": %.*f",
								 perfprop.Decimals, alrelevval);
#endif /* DEBUG */
						STRADD(output, tmpbuf);

						tmpstr = mystrclean(perfprop.Valunit, 4);
						UCS2ToMbs(&valunit, tmpstr,
								  mystrlen(tmpstr)); /* valunit is defined as
														RFC_CHAR length 4 */
						FREE(tmpstr);
						...
						/* SAP uses thresholds for state changes, but we should
						 * assume that the grapher cannot cope with ranges, so
						 * we extract the warning and critical thresholds using
						 * the following logic:
						 *
						 *  In status OK:    Warn=green->yellow,
						 * Crit=yellow->red In status WARN:  Warn=yellow->green,
						 * Crit=yellow->red In status CRIT:  Warn=yellow->green,
						 * Crit=red->yellow
						 *
						 * But if it can then use the '@x:y' format, which is
						 * also not correct but brings us closer...
						 */
						...
						perfstr = mteptr2->name; /* default is to show the full
													perfdata */
						if (trim > 0) {
							i = 0;
							while (*perfstr++ && i < trim) {
								if (*perfstr == PATH_SEPARATOR)
									i++;
							}
							if (!*perfstr)
								perfstr =
									mteptr2
										->name; /* ensure we show something */
						}

						strcpy_nospaces(valstr, perfstr);
						if (flag_ranges) {
							snprintf(tmpbuf, sizeof(tmpbuf),
									 "%s=%.*f%s;@%.*f:%.*f;@%.*f:%.*f ", valstr,
									 perfprop.Decimals, alrelevval,
									 valunit ? valunit : "", perfprop.Decimals,
									 treshy2g, perfprop.Decimals, treshg2y,
									 perfprop.Decimals, treshr2y,
									 perfprop.Decimals, treshy2r);
						} else if (flag_thresholds) {
							snprintf(tmpbuf, sizeof(tmpbuf),
									 "%s=%.*f%s;%.*f;%.*f ", valstr,
									 perfprop.Decimals, alrelevval,
									 valunit ? valunit : "", perfprop.Decimals,
									 warn_threshold, perfprop.Decimals,
									 crit_threshold);
						} else {
							snprintf(tmpbuf, sizeof(tmpbuf), "%s=%.*f%s ",
									 valstr, perfprop.Decimals, alrelevval,
									 valunit ? valunit : "");
						}
						FREE(valunit);
						FREE(valstr);
					...
					} else if (!memcmp(mteptr2->tid.Mtclass,
									   MT_CLASS_SINGLE_MSG,
									   sizeof(mteptr2->tid.Mtclass))) {
					...
					if (output_lines > 1) {
						...
						if (status_count[3]) {
							snprintf(output, sizeof(output),
									 "[%s] %d criticals and %d warnings",
									 exitval[exitcode], status_count[3],
									 status_count[2]);
						} else if (status_count[2]) {
							snprintf(output, sizeof(output), "[%s] %d warnings",
									 exitval[exitcode], status_count[2]);
						} else {
							snprintf(output, sizeof(output),
									 "[%s] %d matching lines found",
									 exitval[exitcode], output_lines);
						}
					}
				}
				mteptr2++;
		...
		if (status_count[0] + status_count[1] + status_count[2] +
				status_count[3] ==
			0) {
			exitcode = 0;
			snprintf(output, sizeof(output), "[%s] No data available",
					 exitval[exitcode]);
		}
...
/* this function calls mbstowcs to convert a string to a 4 byte wide-character
 * string (UCS-4) and then packs it into UCS-2 (2 byte)
 *
...
/* this function packs a UCS-2 or UCS-4 string into a locale dependent character
 * string (e.g. UTF-8)
 *
 * This is a workaround to allow us to use glibc functions compiled with long
 * wchar
 *
 * Assumptions:
...
int UCS4LEToUTF8(unsigned char **dest, const unsigned char *src, size_t len) {
	...
	for (i = 0; i < len; i++) {
		val = (*(from + 3) << 24) + (*(from + 2) << 16) + (*(from + 1) << 8) +
			  (*from);
...
int get_info(RFC_HANDLE handle) {
	...
	func_param params[] = {{NULL, RFCTYPE_NULL, NULL, 0}};
	...
	bind_struct_to_parameter(&importing[0], import_name, &rfcsi, sizeofR(rfcsi),
							 RFCTYPE_CHAR);
#endif

	rfc_rc = call_function(handle, rfc_func, exporting, importing, tables,
						   &error_info);
	...
	if (RfcGetStructure(rfc_func, import_name, &rfc_struct, &error_info))
		show_error(rfc_rc, function, error_info);
	if (RfcGetString(rfc_struct, cU("RFCSAPRL"), message, 256, &message_len,
					 &error_info))
		show_error(rfc_rc, function, error_info);
#endif
 ...
 *
 * It is important to clear the exports array as we search it to look for
 * existing structures And make sure it is large enough to contain all the
 * variables
 *
 ...
 */
int add_export_param(RFC_FUNCTION_HANDLE rfc_func, func_param *param,
					 RFC_PARAMETER *exports) {
	size_t len;
	size_t nlen;
...
#ifdef HAVE_NETWEAVER
	if (RfcGetString(rfc_func, imports[0].name, session_id, sizeofU(session_id),
					 &message_len, &error_info))
		show_error(rfc_rc, function, error_info);
#else
...
int xmi_getmslist(RFC_HANDLE handle, SAP_UC *user) {
	...
	RFC_FUNCTION_HANDLE rfc_func;
	func_param params[] = {{cU("EXTERNAL_USER_NAME"), RFCTYPE_CHAR, user, 0},
						   {NULL, RFCTYPE_NULL, NULL, 0}};
	...
	bind_struct_to_table(&tables[0], table_name, rfc_table, sizeofR(BAPIMSNAME),
						 typeHandle);
#endif
...
int xmi_getmonlist(RFC_HANDLE handle, SAP_UC *user, SAP_UC *set) {
...
#ifndef HAVE_NETWEAVER
	for (export = exporting; export->nlen; export ++) {
	} /* find next free export */
	rfc_rc = BUILD_UNICODE_STRUCT(BAPIMSNAME, typeHandle_BAPIMSNAME);
	bind_struct_to_parameter(export, cU("MONI_SET_NAME"), &Moni_set_name,
							 sizeofR(BAPIMSNAME), typeHandle_BAPIMSNAME);
	...
	bind_struct_to_table(&tables[0], table_name, rfc_table, sizeofR(BAPIMNNAME),
						 typeHandle_BAPIMNNAME);
#endif
	...
	for (i = 0; i < count; i++) {
		...
		tmpstr = mystrclean(Monitor_names.Moni_name,
							sizeofU(Monitor_names.Moni_name));
		tmpstr2 =
			mystrclean(Monitor_names.Ms_name, sizeofU(Monitor_names.Ms_name));
#endif
		printfU(cU("%s / %s\n"), tmpstr2, tmpstr);
		FREE(tmpstr);
		FREE(tmpstr2);
...
int get_ancestor(int tree_items, BAPITNDEXT *tree, int idx, char **name) {
	...
	cleanstr =
		mystrclean(tree[idx - 1].Mtnameshrt, sizeofU(tree[idx - 1].Mtnameshrt));
...
int matchitems(int tree_items, BAPITNDEXT *tree, MTE **mteptrs,
			   wchar_t *match2) {
	char *match, *test;
	wchar_t *cleanstr;
	...
	for (i = 0; i < tree_items; i++) {
		...
		if (flag_fullpath) {
			if (tree[i].Alparintre > 0) {
				...
				while (parent_id = get_ancestor(tree_items, tree, parent_id,
												&full_name)) {
				}
				FREE(test);
				test = full_name;
...
#endif /* DEBUG */
		printfU(cU("%*s%.*s"), tree[i].Allevintre * 2, cU(""),
				sizeofU(tree[i].Mtnameshrt), tree[i].Mtnameshrt);
#ifdef DEBUG
		printfU(cU("\t\t(child of %d, level %d, class is %.*s)"),
				tree[i].Alparintre, tree[i].Allevintre,
				sizeofU(tree[i].Mtclass), tree[i].Mtclass);
#endif /* DEBUG */
		printfU(cU("\n"));
	}
}
 ...
 */
int xmi_getmontree(RFC_HANDLE handle, SAP_UC *user, SAP_UC *set, SAP_UC *mon,
				   BAPITNDEXT **tree) {
	RFC_RC rfc_rc = RFC_OK;
	RFC_ERROR_INFO error_info;
	...
	RFC_FUNCTION_HANDLE rfc_func;
	func_param params[] = {{cU("EXTERNAL_USER_NAME"), RFCTYPE_CHAR, user, 0},
						   {cU("MONITOR_NAME.MONI_NAME"), RFCTYPE_CHAR, mon,
							0}, /* member of struct */
						   {cU("MONITOR_NAME.MS_NAME"), RFCTYPE_CHAR, set,
							0}, /* member of struct */
						   {NULL, RFCTYPE_NULL, NULL, 0}};
	...
	rfc_rc = BUILD_UNICODE_STRUCT(BAPIMNNAME, typeHandle_BAPIMNNAME);
	bind_struct_to_parameter(export, cU("MONITOR_NAME"), &Monitor_name,
							 sizeofR(BAPIMNNAME), typeHandle_BAPIMNNAME);
	...
	bind_struct_to_table(&tables[0], table_name, rfc_table, sizeofR(BAPITNDEXT),
						 typeHandle_BAPITNDEXT);
#endif
	...
	rfc_rc = RfcGetParameterDescByName(rfc_func_desc, table_name, &desc,
									   &error_info);
	printfU(cU("%d: %s\n"), i, desc.name);
	// get and list fields available
	rfc_rc = RfcGetFieldCount(desc.typeDescHandle, &count, &error_info);
	for (i = 0; i < count; i++) {
		rfc_rc = RfcGetFieldDescByIndex(desc.typeDescHandle, i, &field_desc,
										&error_info);
		if (rfc_rc != RFC_OK) {
			show_error(rfc_rc, function, error_info);
		}
...
int tree_to_cache(SAP_UC *set, SAP_UC *mon, int number_of_items,
				  BAPITNDEXT **tree) {
	char cachefile[1024];
	 ...
	 */
	if (!(getuid()))
		return 0; /* refuse to create cache as root */

	memset(cachefile, 0, sizeof(cachefile));
	if (snprintf(cachefile, sizeof(cachefile), "/tmp/%s_%s_%d", set, mon,
				 number_of_items) >= sizeof(cachefile))
		return 0;
}
#endif /* CACHE */

int xmi_get_perfdata(RFC_HANDLE handle, SAP_UC *user, BAPITID item_tid,
					 BAPIPACVAL *retstr) {
	RFC_RC rfc_rc = RFC_OK;
	RFC_ERROR_INFO error_info;
	...
	RFC_FUNCTION_HANDLE rfc_func;
	func_param params[] = {{cU("EXTERNAL_USER_NAME"), RFCTYPE_CHAR, user, 0},
						   {cU("TID.MTSYSID"), RFCTYPE_CHAR, item_tid.Mtsysid,
							0}, /* member of struct */
						   {cU("TID.MTCLASS"), RFCTYPE_CHAR, item_tid.Mtclass,
							0}, /* member of struct */
						   {cU("TID.MTNUMRANGE"), RFCTYPE_CHAR,
							item_tid.Mtnumrange, 0}, /* member of struct */
						   {cU("TID.MTMCNAME"), RFCTYPE_CHAR, item_tid.Mtmcname,
							0}, /* member of struct */
						   {cU("TID.MTUID"), RFCTYPE_CHAR, item_tid.Mtuid,
							0}, /* member of struct */
						   {NULL, RFCTYPE_NULL, NULL, 0}};
	...
	bind_struct_to_parameter(export, cU("TID"), &item_tid, sizeofR(item_tid),
							 typeHandle_BAPITID);

	for (import = importing; import->nlen; import++) {
	} /* find next free import */
	rfc_rc = BUILD_UNICODE_STRUCT(BAPIPACVAL, typeHandle_BAPIPACVAL);
	bind_struct_to_parameter(import, import_name, &values, sizeofR(values),
							 typeHandle_BAPIPACVAL);
#endif
...
int xmi_get_perfprop(RFC_HANDLE handle, SAP_UC *user, BAPITID item_tid,
					 BAPIPAPROP *retstr) {
	RFC_RC rfc_rc = RFC_OK;
	RFC_ERROR_INFO error_info;
...
int xmi_get_status_msg(RFC_HANDLE handle, SAP_UC *user, BAPITID item_tid,
					   BAPISMCVAL *retstr) {
	RFC_RC rfc_rc = RFC_OK;
	RFC_ERROR_INFO error_info;
...
int xmi_get_value(RFC_HANDLE handle, SAP_UC *user, BAPITID item_tid,
				  BAPIALDATA *retstr) {
	RFC_RC rfc_rc = RFC_OK;
	RFC_ERROR_INFO error_info;
...
int xmi_logoff(RFC_HANDLE handle) {
	...
	rfc_rc = RfcCallReceive(handle, function, exporting, importing, tables,
							&error_info);
...
unsigned build_param_nw(RFC_CONNECTION_PARAMETER (*param)[],
						struct sap_options_s *opt) {

	char **tmp;
	char gwserv[9];
	unsigned cnt = 0;

	(*param)[cnt].name = cU("user");
	(*param)[cnt++].value = toU16(opt->user);
	(*param)[cnt].name = cU("passwd");
	(*param)[cnt++].value = toU16(opt->passwd);
	(*param)[cnt].name = cU("lang");
	(*param)[cnt++].value = toU16(opt->lang);
	(*param)[cnt].name = cU("client");
	(*param)[cnt++].value = toU16(opt->client);
	/*
		(*param)[cnt].name = cU("type"); (*param)[cnt++].value =
	   toU16(opt->ver);
		(*param)[cnt].name = cU("trace"); (*param)[cnt++].value =
	   toU16(opt->trace);
	*/
	(*param)[cnt].name = cU("trace");
	(*param)[cnt++].value = opt->trace ? cU("1") : cU("0");

	if (opt->ver > 2) {
		(*param)[cnt].name = cU("ashost");
		(*param)[cnt++].value = toU16(opt->ashost);
		(*param)[cnt].name = cU("sysnr");
		(*param)[cnt++].value = toU16(opt->sysnr);
	} else {
		if (!opt->gwserv) {
			snprintf(gwserv, sizeof(gwserv), "sapgw%s", opt->sysnr);
		}
		(*param)[cnt].name = cU("dest");
		(*param)[cnt++].value = toU16(opt->dest);
		(*param)[cnt].name = cU("gwhost");
		(*param)[cnt++].value = toU16(opt->gwhost);
...
int build_param(char **param, struct sap_options_s *opt) {
	...
	asprintf(param, "TYPE=%d USER=%s PASSWD=%s CLIENT=%s LANG=%s TRACE=%d",
			 opt->ver, quote_param(opt->user), quote_param(opt->passwd),
			 opt->client, opt->lang, opt->trace);

	if (opt->ver > 2) {
		asprintf(param, "%s ASHOST=%s SYSNR=%s", *param, opt->ashost,
				 opt->sysnr);
	} else {
		if (!opt->gwserv) {
			snprintf(gwserv, sizeof(gwserv), "sapgw%s", opt->sysnr);
		}
		asprintf(param, "%s DEST=%s GWHOST=%s GWSERV=%s", *param,
				 quote_param(opt->dest), opt->gwhost,
				 opt->gwserv ? opt->gwserv : gwserv);
	}
	return 0;
}
...
int usage(char *me) {
	...
	printf("  --gwserv|-g\t\tSAP Gateway Server (V2 - defaults to sapgwxx "
		   "where xx is the sysnr)\n");
	printf("\n");
	printf("  --fullpath|-f\t\tMatch against the full path of the Object "
		   "(Separator is %c )\n",
		   PATH_SEPARATOR);
	printf("  --trim\t\tRemove this number of leading parts from the "
		   "performance data (useful with fullpath)\n");
	printf("  --thresholds\t\tInclude the SAP thresholds in the perf data "
		   "(simplified)\n");
	printf("  --use-ranges\t\tInclude the SAP thresholds in the perf data "
		   "(range format)\n");
#ifdef CACHE
	printf("  --cache\t\tEnable cacheing of the monitor tree for this number "
		   "of seconds\n");
#endif /* CACHE */
	printf("\n  --mode|-m\t\tBeing one of: ");
	for (i = 0; modes[i]; i++) {
 ...
 */
RFC_FUNCTION_HANDLE prepare_function(RFC_HANDLE handle, SAP_UC *function,
...
RFC_RC call_function(RFC_HANDLE handle, RFC_FUNCTION_HANDLE rfc_func,
					 RFC_PARAMETER *exporting, RFC_PARAMETER *importing,
					 RFC_TABLE *tables, RFC_ERROR_INFO *error_infop) {
	RFC_RC retval;
#ifndef HAVE_NETWEAVER
	retval = RfcCallReceive(handle, rfc_func, exporting, importing, tables,
							error_infop);
#else
...
#ifndef HAVE_NETWEAVER
int bind_struct_to_table(RFC_TABLE *table_entry, SAP_UC *table_name,
						 RFC_TABLE_HANDLE rfc_table, size_t size_of_struct,
...
int bind_struct_to_parameter(RFC_PARAMETER *parameter_entry,
							 SAP_UC *parameter_name, RFCSI *rfc_sip,
...
int fill_table(RFC_FUNCTION_HANDLE rfc_func, SAP_UC *table_name,
			   RFC_TABLE_HANDLE *rfc_tablep, RFC_ERROR_INFO *error_infop) {
#ifdef HAVE_NETWEAVER
...
int fill_structure(DATA_CONTAINER_HANDLE handle,
				   RFC_UNICODE_TYPE_ELEMENT *descr, unsigned count,
				   void *values) {
	int i;
	...
	for (i = 0; i < count; i++) {
		...
		if (descr->type == RFCTYPE_INT) {
			...
			*(SAP_INT *)(values + descr->c2_offset) = intval;
			...
			rfc_rc = RfcGetChars(handle, upper, strval, sizeofU(strval) - 1,
								 &error_info);
			if (rfc_rc) {
